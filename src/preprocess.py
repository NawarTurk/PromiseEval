...
def preprocess():
    ...
    for language in languages:
        ...
        raw_dataset_path = os.path.join(base_path, 'dataset', '1-raw-data', f'PromiseEval_Trainset_{language}.json')
        ...
        for item in raw_dataset:
            ...
            sentiment = get_sentiment_analysis(text, language)
        ...
        with open(os.path.join(processed_data_path, f'{language}_task1.json'), 'w', encoding='utf-8') as f:
            json.dump(task1_json, f, ensure_ascii=False, indent=2)
        
        with open(os.path.join(processed_data_path, f'{language}_task2.json'), 'w', encoding='utf-8') as f:
        ...
        task1_df.to_csv(os.path.join(processed_data_path, f'{language}_task1-processed.csv'), index=False)
        task1_train_df.to_csv(os.path.join(processed_data_path, f'{language}_task1-train.csv'), index=False)
        task1_test_df.to_csv(os.path.join(processed_data_path, f'{language}_task1-test.csv'), index=False)
        
        task2_df.to_csv(os.path.join(processed_data_path, f'{language}_task2-processed.csv'), index=False)
        task2_train_df.to_csv(os.path.join(processed_data_path, f'{language}_task2-train.csv'), index=False)
        task2_test_df.to_csv(os.path.join(processed_data_path, f'{language}_task2-test.csv'), index=False)

        print(f"Preprocessing and splitting {language} completed successfully.")
